...
function setupTeaserFlipVideo() {
    ...
    function updateVideoSize() {
        let currentHeight = toNumber(card.style.getPropertyValue('--teaser-card-height')) ||
            front.getBoundingClientRect().height;
        if (!currentHeight) return;

        const styles = window.getComputedStyle(videoCard);
        ...
        const contentHeight = Math.max(1, currentHeight - verticalInset);
        let targetWidth = Math.ceil(contentHeight * videoAspect() + horizontalInset);
        const maxWidth = Math.max(1, Math.min(
            window.innerWidth - 32,
            shell.parentElement ? shell.parentElement.getBoundingClientRect().width : window.innerWidth
        ));

        if (targetWidth > maxWidth) {
            const contentWidth = Math.max(1, maxWidth - horizontalInset);
            currentHeight = Math.ceil(contentWidth / videoAspect() + verticalInset);
            targetWidth = Math.ceil(contentWidth + horizontalInset);
        }

        card.style.setProperty('--teaser-card-height', `${currentHeight}px`);
        shell.style.setProperty('--teaser-video-width', `${targetWidth}px`);
    }

    function flipToVideo() {
        if (shell.classList.contains('is-video')) return;

        const currentHeight = front.getBoundingClientRect().height;
        if (currentHeight) {
            card.style.setProperty('--teaser-card-height', `${currentHeight}px`);
        }
        updateVideoSize();
